...
This report compares two fundamentally different modeling paradigms---Gradient
Boosted Decision Trees (GBDT) and Multi-Layer Perceptrons (MLP)---on a binary
classification task: predicting whether a client of a Portuguese bank will
subscribe to a term deposit after a phone-based marketing campaign.

The \textbf{Bank Marketing} dataset is provided in two versions: an extended
version with 20 input features, and a classic version with 17 features (16
inputs + 1 target). Since the dataset documentation only provides detailed
descriptions for the 17-feature version, we opted to use the classic file
(\texttt{bank\text{-}full.csv}) to ensure every feature is well-understood and
interpretable. The selected dataset contains 45,211 records with 16 input
features spanning client demographics
(age, job, marital status, education), financial indicators (balance, credit
default, housing/personal loans), and campaign attributes (contact type, month,
number of contacts, outcome of previous campaigns). The target variable
\texttt{y} is heavily imbalanced: only 11.7\% of clients subscribed (``yes''),
while 88.3\% did not (``no'').

This controlled experiment---same data, same splits, same evaluation---lets us
directly compare how each model family learns, what preprocessing it demands,
and where it excels or struggles.
...
No columns contained null values, but several categorical features
(\texttt{job}, \texttt{education}, \texttt{contact}, \texttt{poutcome}) used
the string \texttt{"unknown"} as a placeholder. We considered three strategies:
\begin{enumerate}[nosep]
    \item \textbf{Keep ``unknown'' as its own category} --- simplest; may carry
          signal (clients who withhold information may behave differently).
    \item \textbf{Impute with mode} --- replaces unknowns with the most frequent
          value, but discards the ``unknown'' signal.
    \item \textbf{Drop rows with unknowns} --- cleanest data but loses
          ${\sim}25\%$ of rows.
\end{enumerate}
We chose option~1 because ``unknown'' itself is informative: for instance,
clients reached via an unknown contact method disproportionately came from
older campaign records with lower subscription rates.
...
The \texttt{duration} feature (last contact duration in seconds) was dropped.
While the dataset authors note it is highly predictive, it is only available
\emph{after} a call ends and therefore cannot be used in a realistic deployment
scenario. Removing it forces the models to learn from features that are
available before the campaign call is made.

\paragraph{Encoding.}
We considered three encoding strategies: (a)~one-hot encoding, (b)~label
encoding, and (c)~a mix (label for binary columns, one-hot for multi-class).
We chose \textbf{one-hot encoding with \texttt{drop\_first=True}} for all
categoricals. This avoids implying ordinal relationships (e.g., ``admin''
$<$ ``student'') that could mislead gradient-based MLP training, while the
dropped reference category prevents multicollinearity. The result was 47 input
features.
...
A \texttt{StandardScaler} was \textbf{fit on the training set only} and then
applied to validation and test sets, preventing data leakage.

Scaling is essential for MLP because its neurons compute weighted sums of
inputs followed by non-linear activations; features on vastly different scales
(e.g., \texttt{balance} ranging to 102,127 vs.\ binary indicators) elongate
the loss surface, causing slow, unstable gradient descent. Standardization
(mean$=0$, std$=1$) makes the loss landscape more isotropic.

Decision trees, by contrast, split on individual features using thresholds
(``is balance $> 500$?''). These splits are invariant to monotonic
transformations---whether balance ranges from 0--100,000 or is standardized to
$-2$ to $+3$, the tree finds the same optimal split. Scaling is therefore
unnecessary for GBDT.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{fig_data_preparation.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{fig_xgb_loss_curve.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{fig_xgb_learning_rate.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{fig_xgb_feature_importance.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{fig_mlp_loss_balanced_vs_imbalanced.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{fig_mlp_architecture_comparison.png}
    \caption{Training loss curves for different MLP architectures. Deeper
             networks achieve lower training loss but do not generalize better
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{fig_mlp_lr_activation.png}
    \caption{Training loss curves for different learning rates and activation
             functions. Higher learning rates converge faster but with more
             noise; tanh converges more smoothly but achieves worse F1.}
    \label{fig:mlp_lr}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{fig_mlp_final_loss.png}
    \caption{Training loss and validation score for the final tuned MLP.}
    \label{fig:mlp_final}
\end{figure}
...
GBDT reduces bias through \textbf{sequential boosting}---each new tree
corrects the residual errors of the ensemble so far---and controls variance
through regularization (tree depth limits, row/column subsampling, L1/L2
penalties). This combination is well-suited to medium-sized tabular data.

MLP reduces bias through \textbf{flexible function approximation} (universal
approximation theorem) but is prone to high variance, especially with limited
data and class imbalance. The oversampling strategy addressed the imbalance
but introduced its own variance: duplicated minority samples can cause the
network to memorize specific instances rather than learning generalizable
patterns. This was reflected in the inflated CV F1 (0.93 on balanced data)
versus the much lower test F1 (0.31 on real data).

\subsection{Key lessons learned}

\begin{enumerate}[nosep]
    \item \textbf{Class imbalance must be handled explicitly.} For GBDT,
          \texttt{scale\_pos\_weight} provided a clean, built-in solution.
          For MLP, the lack of a \texttt{class\_weight} parameter forced us
          to oversample, which was effective but added pipeline complexity
          and inflated cross-validation scores.
    \item \textbf{Accuracy is misleading under imbalance.} MLP achieved higher
          accuracy (83.3\%) than GBDT (82.3\%), but this was entirely due to
          more aggressive ``No'' predictions. F1, precision--recall, and
          AUC-PR are far more informative.
    \item \textbf{Simpler architectures can outperform deeper ones.} For MLP,
          adding layers increased accuracy but \emph{decreased} F1---the extra
          capacity was used to predict the majority class more confidently.
    \item \textbf{Feature scaling is non-negotiable for neural networks.}
          Without standardization, MLP convergence is slow and unstable. Trees
